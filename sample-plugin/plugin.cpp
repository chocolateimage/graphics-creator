...
bool render(PluginInterface *intf, PluginEffectRenderContext *renderContext,
  ...
  for (int y = 0; y < sourceRect.h; y++) {
    for (int x = 0; x < sourceRect.w; x++) {
      ...
      Color c = intf->functions->getBrushPixel(foregroundBrush, x, y,
...
int gcPluginInit(PluginInterface *intf, PluginInitData *data) {
  ...
  intf->functions->addEffectProperty(effect, PROPERTY_TYPE_COLOR, "background");
  intf->functions->addEffectProperty(effect, PROPERTY_TYPE_BRUSH, "foreground");
